...
const { Comment, Post } = require("../../d_schemas"); // 테스트 과정에서 DB를 직접 확인하는 경우가 있으므로 DB 모델을 직접 임포트
...
beforeAll(async () => {
  ...
  // 위 2명 각각 로그인 ---------------
  const response1 = await request(app).post("/api/login").send(userDataIn.loginpReq1);
  cookie_1 = response1.headers["set-cookie"];

  const response3 = await request(app).post("/api/login").send(userDataIn.loginpReq3);
  cookie_3 = response3.headers["set-cookie"];

  // 게시글 각 3개 작성 -----------------
  await request(app).post("/api/posts").set("Cookie", cookie_1).send(postDataIn.sampleBaseReq_1);

  await request(app).post("/api/posts").set("Cookie", cookie_1).send(postDataIn.sampleBaseReq_2);

  await request(app).post("/api/posts").set("Cookie", cookie_3).send(postDataIn.sampleBaseReq_3);
...
describe("#1 POST api/comments/:_postId 댓글 작성 셍성 테스트", () => {
  ...
  test("1번 게시글에 댓글 작성 - 작성된 DB의 내용 일치 확인 ", async () => {
    const response = await request(app).post("/api/comments/1").set("Cookie", cookie).send(commentDataIn.createCommentReq);
...
describe("#3 PUT /api/comments/:_commentId 특정 댓글 수정", () => {
  ...
  beforeEach(async () => {
    ...
    const response = await request(app).post("/api/login").send(userDataIn.loginpReq1); // 1번 댓글을 작성한 Tester1로 로그인 (user "Tester1")
